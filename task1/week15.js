...
for (let i = 0; i < vowels.length; i++) {
  if (greeting.includes(vowels[i])) {
    vowelCount += 1;
    let index = greeting.indexOf(vowels[i]);
    if ((greeting.includes(vowels[i]), index + 1)) {
      vowelCount += 1;
    }
  }
}
...
let p = 0;
while (p < allNumbers.length) {
  if (allNumbers[p] < 0) {
    console.log(`Не все элементы массива положительные`);
    break;
  }
  p++;
}
...
let sumNum = 0;
do {
  sumNum = +prompt("Вводите числа, пока сумма чисел не станет больше 100", 0);
} while (sumNum <= 100);
...
changeColor(document.getElementsByTagName("h4"), "blue");
//Задание 24
